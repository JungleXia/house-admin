...
<fieldset class="layui-elem-field">
    ...
    <form class="layui-form">
        <div class="layui-inline">
            <input type="text" value="" name="s_callerNum" placeholder="请输入手机号" class="layui-input search_input">
        </div>
        <div class="layui-inline">
...
<div class="layui-form users_list">
    ...
    <script type="text/html" id="isDownloaded">
    	{{#  if(d.downloadUrl != null && d.downloadUrl != ''){ }}
	        <!-- 这里的 checked 的状态只是演示 -->
	        {{#  if(d.isDownloaded == true){ }}
    ...
    <script type="text/html" id="ossUrl">
    	{{#  if(d.ossUrl != null && d.ossUrl != ''){ }}
        	<a class="layui-badge layui-bg-green" download="{{d.ossUrl}}">下载</a>
         {{#  } }}
    </script>
    <script type="text/html" id="download">
    	{{#  if(d.downloadUrl != null && d.downloadUrl != ''){ }}
       		<a class="layui-badge layui-bg-orange" target="_blank" href="{{d.downloadUrl}}">录音文件</a>
       	{{#  } }}
    </script>
...
<script>
    layui.use(['layer','form','table'], function() {
        ...
        t = {
            elem: '#test',
            url:'${base}/admin/user/telbill/query',
            method:'post',
            ...
            cols: [[
                {type:'checkbox'},
                {field:'callerNum',    title: '拨打电话'},
                {field:'calleeNum',    title: '收听电话'},
                {field:'bindNum',    title: '加密电话'},
                {field:'subscriptionId',    title: '订单id'},
                {field:'callInTime',    title: '呼入时间'},
                {field:'callEndTime',    title: '结束时间'},
                {field:'recordFlag',    title: '录音标志'},
                {field:'isDownloaded',    title: '下载状态', templet: '#isDownloaded'},
                {field:'downloadUrl',    title: '华为云（只保留7天）', templet: '#download'},
                {field:'ossUrl',    title: '阿里云', templet: '#showUrl'},
            ]]
        };
        table.render(t);
