...
<fieldset class="layui-elem-field">
    ...
    <form class="layui-form">
         <div class="layui-inline">
            <input type="text" value="" name="s_phoneNumber" placeholder="请输入手机号" class="layui-input search_input">
        </div>
...
<div class="layui-form users_list">
    ...
    <script type="text/html" id="progress">
		...
		{{#  } else if(d.ticket == true && d.viewTicket == false) { }}
         ...
         {{#  } else if(d.freeMove == true && d.viewFreeSer == false){ }}
         ...
         {{# } else if((d.viewTicket == true || d.viewFreeSer == true) && d.upload == false) { }}
         <div class="layui-progress-bar layui-bg-red" lay-percent="75%" style="width: 75%"><span class="layui-progress-text">已查看短信链接</span></div>
    ...
    <script type="text/html" id="result">
    	{{#if (d.result == 0 && d.errMsg == 'OK') { }}
    		<a class="layui-badge layui-bg-green" target="_blank">成功 </a>
    	{{# } else if(d.result != 0){ }}
    		<a class="layui-badge layui-bg-red" href="https://cloud.tencent.com/document/product/382/3771" target="_blank">{{d.result}}</a>
    	{{# } }}
    </script>
</div>
<div id="page"></div>
...
<script>
    layui.use(['layer','form','table', 'element'], function() {
        ...
        t = {
            ...
            cols: [[
                {type:'checkbox'},
                {field:'templateId',     title: '模板ID'},
                {field:'phoneNumber',  title: '号码'},
                {field:'result',    	title: '响应结果', templet: '#result'},
                {field:'errMsg',    	title: '响应信息'},
                {field:'fee',	title: '扣费'},
        ...
        table.on('tool(demo)', function(obj){
            var data = obj.data;
            if(obj.event === 'edit'){
                layer.confirm("你确定重发短信？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${base}/admin/rent/sms/resend",{"id":data.id},function (res){
                                if(res.success){
