...
<div class="layui-form users_list">
    ...
    <script type="text/html" id="registerStatus">
        <!-- 这里的 checked 的状态只是演示 -->
        {{#  if(d.register == true){ }}
        <span class="layui-badge layui-bg-green">注册</span>
        {{#  } else { }}
        <span class="layui-badge layui-bg-gray">未注册</span>
        {{#  } }}
    </script>
    <script type="text/html" id="allowLocate">
        {{#  if(d.allowLocate == true){ }}
        <span class="layui-badge layui-bg-green">允许</span>
        {{#  } else { }}
        <span class="layui-badge layui-bg-gray">未允许</span>
        {{#  } }}
    </script>
    <script type="text/html" id="allowDevice">
        {{#  if(d.allowDevice == true){ }}
        <span class="layui-badge layui-bg-green">允许</span>
        {{#  } else { }}
        <span class="layui-badge layui-bg-gray">未允许</span>
        {{#  } }}
    </script>
     <script type="text/html" id="modifysf">
        {{#  if(d.modifysf == true){ }}
        <span class="layui-badge layui-bg-green">修改</span>
        {{#  } else { }}
        <span class="layui-badge layui-bg-gray">未修改</span>
        {{#  } }}
    </script>
     <script type="text/html" id="csite">
        {{#  if(d.csite == 40001){ }}
        <span class="layui-badge layui-bg-green">抖音</span>
        {{#  } }}
    </script>
    <script type="text/html" id="modifyother">
        <!-- 这里的 checked 的状态只是演示 -->
        {{#  if(d.modifyother == true){ }}
        <span class="layui-badge layui-bg-green">修改</span>
        {{#  } else { }}
        <span class="layui-badge layui-bg-gray">未修改</span>
        {{#  } }}
...
<script>
    layui.use(['layer','form','table'], function() {
        ...
        t = {
            elem: '#test',
            url:'${base}/admin/user/devicesList',
            method:'post',
            ...
            cols: [[
                ...
                {field:'os',    title: '手机型号'},
                {field:'csite',    title: '来源', templet: '#csite'},
                {field:'idfa',    title: 'IDFA'},
                {field:'imei',    title: 'IMEI'},
                {field:'nextfp',    title: '操作换一批次数'},
                {field:'register',    title: '是否注册', templet:'#registerStatus'},
                {field:'allowLocate',    title: '允许定位', templet: '#allowLocate'},
                {field:'allowDevice',    title: '获取设备', templet: '#allowDevice'},
                {field:'modifysf',    title: '修改首付', templet: '#modifysf'},
                {field:'modifyother',    title: '修改其他', templet: '#modifyother'},
                {field:'createTime',    title: '登入时间'},
            ]]
        };
        table.render(t);
