...
<head>
    <meta charset="utf-8">
    <title>${city.city}市区域列表--${site.name}</title>
...
<fieldset class="layui-elem-field">
    <legend>${city.city}区域检索</legend>
    <div class="layui-field-box">
    <form class="layui-form">
        <div class="layui-inline">
            <input type="text" value="" name="s_district" placeholder="请输入区域" class="layui-input search_input">
        </div>
        <div class="layui-inline">
        ...
        <div class="layui-inline">
            <a class="layui-btn layui-btn-normal" data-type="addUser">添加区域</a>
        </div>
    </form>
    </div>
</fieldset>
...
<script>
    layui.use(['layer','form','table'], function() {
        ...
        t = {
            elem: '#test',
            url:'${base}/admin/fang/region/district/list',
            method:'post',
            ...
            cols: [[
                ...
                {field:'districtCode',  title: '行政区编码'},
                {field:'city',    	title: '城市'},
        ...
        table.on('tool(demo)', function(obj){
            ...
            if(obj.event === 'edit'){
                var editIndex = layer.open({
                    title : "编辑${city.city}区域",
                    type : 2,
                    area: ['50%', '60%'],
                    content : "${base}/admin/fang/region/district/edit?id="+data.id,
            ...
            if(obj.event === "del"){
                layer.confirm("你确定要删除该区域么？",{btn:['是的,我确定','我再想想']},
                        function(){
                            $.post("${base}/admin/fang/region/district/deleteById",{"id":data.id},function (res){
                                if(res.success){
        ...
        var active={
            addUser : function(){
                var addIndex = layer.open({
                    title : "添加${city.city}区域",
                    type : 2,
                    area: ['50%', '60%'],
                    content : "${base}/admin/fang/region/district/add?cityId=${city.id}",
