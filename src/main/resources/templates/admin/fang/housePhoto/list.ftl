...
<fieldset class="layui-elem-field">
    ...
    <form class="layui-form">
        ...
        <div class="layui-inline">
            <a class="layui-btn layui-btn-normal" data-type="addUser">批量下载图片</a>
        </div>
    </form>
    </div>
</fieldset>
<div class="layui-form users_list">
    ...
    <script type="text/html" id="showPhoto">
    	{{#  if(d.source != null){ }}
		<img src="{{d.source}}" width='50' height='50' onclick="layerPhoto('{{d.source}}', '{{d.size}}')">
		{{#  } }}
    </script>
    <script type="text/html" id="showTarget">
    	{{#  if(d.target != null){ }}
		<img src="{{d.target}}" width='50' height='50' onclick="layerPhoto('{{d.target}}', '{{d.size}}')">
		{{#  } }}
    </script>
</div>
<div id="page"></div>
...
<script>
    layui.use(['layer','form','table'], function() {
        ...
        t = {
            elem: '#test',
            url:'${base}/admin/fang/housePhoto/list',
            method:'post',
            ...
            cols: [[
                ...
                {field:'source',    title: '原链接', templet:'#showPhoto'},
                {field:'size',    	title: '大小'},
                {field:'large',    	title: '大图'},
                {field:'middle',    title: '中图'},
                {field:'small',    	title: '小图'},
                {field:'target',    title: 'oss', templet:'#showTarget'},
                {field:'dataFrom',  title: '来源'},
                {field:'type',		title: '类型', templet:'#photoType'},
        ...
        var active={
            addUser : function(){
				layer.confirm('是否开始下载图片',  function(index, layero){
					$.post("${base}/admin/fang/housePhoto/download", null ,function (res){
                        if(res.success){
                            layer.msg("正在下载图片",{time: 1000},function(){
                                table.reload('test', t);
                            });
                        }else{
                            layer.msg(res.message);
